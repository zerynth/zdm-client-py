...
.. method:: connect()
    ...
    def open_condition(self, uuid, tag, start, payload=None):
        """
.. method:: open_condition(uuid, tag, start, payload)

      Publish a condition to the ZDM.

      * :samp:`uuid`, the id of the condition generated by the device
      * :samp:`tag`, the label associated to the condition
      * :samp:`start`, the start time of the condition (RFC3339 format) (empty to open a condition)
      * :samp:`payload`, the payload message for the condition of the device
    ...
    def close_condition(self, uuid, tag, finish):
        """
.. method:: close_condition(uuid, tag, finish)

      Publish a condition to the ZDM.

      * :samp:`uuid`, the id of the condition generated by the device
      * :samp:`tag`, the label associated to the condition
      * :samp:`finish`, the finish time of the condition (RFC3339 format) (fill to close a condition)
